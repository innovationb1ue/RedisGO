...
func sMembersSet(m *MemDb, cmd [][]byte, conn net.Conn) resp.RedisData {
	...
	members := set.Members()
	res := make([]resp.RedisData, 0, len(members)) // pre allocate enough memory to avoid copy
	for _, member := range members {
		res = append(res, resp.MakeStringData(member))
	}
...
func sMoveSet(m *MemDb, cmd [][]byte, conn net.Conn) resp.RedisData {
	...
	var desSet *Set
	var desExist bool
	// get dest set or create one if not exist
	tem, ok = m.db.Get(desKey)
	if !ok {
		desSet = NewSet()
		desExist = false
